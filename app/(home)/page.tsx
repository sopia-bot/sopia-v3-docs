...
import { ArrowRight, Gift, Music, MessageSquare, Bot, Package, UsersIcon, Quote as QuoteIcon, User as UserIcon, HardDriveDownload as DownloadIcon } from 'lucide-react'; // UsersIcon 추가
...
function Button({ children, variant = 'default', className = '', size = 'default' }: ButtonProps) {
  const baseStyle = 'inline-flex items-center justify-center rounded-md font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring cursor-pointer';
  const variantStyles = {
...
function FeatureCard({ title, description, icon, className }: FeatureCardProps) {
  return (
    <div className={`rounded-lg border border-border bg-card p-6 shadow-sm transition-all hover:shadow-md ${className} min-h-[220px]`}>
...
export default async function HomePage() {
  ...
  const result = await fetch('https://kr-api.spooncast.net/users/2100369').then(res => res.json());
    ...
    <main className="flex flex-1 flex-col items-center py-16 px-4 md:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="container px-4 md:px-6">
          <div className="flex flex-col justify-center space-y-4">
              ...
              <div className="flex flex-col gap-2 min-[400px]:flex-row mt-[50px] justify-center">
                <Link href="/docs">
                  <Button size="lg" className="gap-2 w-full min-[400px]:w-auto bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white shadow-xl transform transition-all hover:scale-105 focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 focus:ring-offset-background px-8 py-3 rounded-lg">
                    시작하기
      ...
      <div className='relative'>
        ...
        <div className='relative rounded-lg top-0 left-0 w-full h-full z-0 banner-border'>
            <Image src={Banner} alt="Banner" className="w-full h-auto rounded-lg"/>
          </div>
        ...
        <div className="container px-4 md:px-6">
          <div className="mx-auto max-w-3xl text-center">
              ...
              <blockquote className="relative z-10 text-xl md:text-2xl italic font-medium leading-relaxed text-card-foreground">
                ...
                방송을 풍성하게 만들 컨텐츠와 다양한 기능이 준비되어 있을 뿐 아니라 
                방송을 똑똑하게 서포트해 주기 때문에 방송의 틀을 잡을 때 큰 도움이 됩니다.
                </p>
              </blockquote>
              <div className="mt-8 flex flex-col items-center text-center">
                ...
                <div>
                  <Link href={'https://www.spooncast.net/kr/channel/2100369/tab/home'} target="_blank" className="text-xl font-bold text-card-foreground hover:underline">{user?.nickname || '연하영'}</Link>
                  <p className="text-md text-muted-foreground">팔로워: {followerStr(user?.follower_count || 0)}</p>
                </div>
              </div>
          </div>
        </div>
      ...
      <section 
        ...
        style={{
          backgroundImage: 'radial-gradient(circle, rgba(255,255,255,0.03) 1px, transparent 1.5px)',
      ...
      >
        <div className="container mx-auto px-4 md:px-6 text-center flex flex-col items-center">
          <div className="mb-6">
            ...
            <svg width="60" height="60" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className="text-blue-500">
          ...
          <p className="mb-4 text-lg text-gray-400">[ 이제, 내가 선택할 차례입니다. ]</p>
          ...
          <h2 className='text-2xl md:text-3xl font-bold mb-10 leading-tight'>
            당신의 목소리는 완벽하단걸<br/>더 많은 사람에게 알려주자구요.
          </h2>
